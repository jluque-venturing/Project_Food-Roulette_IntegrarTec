...
// (el estado del menú de usuario lo maneja initChrome; el corazón se refresca via onUserUpdate)

// ── Arranque ──────────────────────────────────────────────────────

async function init() {
  // Navbar + footer + modal de login + tema + auth (una sola llamada).
  // onUserUpdate refresca el corazón de favoritos al cambiar la sesión.
  const { updateUserUI } = initChrome({
    onUserUpdate: () => { if (activeRecipe) refreshFavButton(activeRecipe); },
  });
